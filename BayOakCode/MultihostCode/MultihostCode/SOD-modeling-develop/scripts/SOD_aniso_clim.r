...
setwd("C:/Users/dagaydos/Documents/Tangible_Landscape_Stakeholder_Workshop/SOD-modeling-develop/SOD-modeling-develop")
...
##to add LIDE infection raster
I_lide_rast = raster(I_lide, xmn=-226991, xmx=-216991, ymn=37177, ymx=47177)
I_umca_rast = raster(I_umca, xmn=-226991, xmx=-216991, ymn=37177, ymx=47177)
...
for (tt in tstep){
  ...
  if (tt == tstep[1]) {
    ...
    ##FOR INFECTED LIDE OUTPUTS!
    ##CALCULATE OUTPUT TO PLOT: 
    # 1) values as % infected
    #I_lide_rast[] <- ifelse(I_lide_rast[] == 0, NA, I_lide_rast[]/lide_rast[])
    
    # 2) values as number of infected per cell
    #I_lide_rast[] <- ifelse(I_lide_rast[] == 0, NA, I_lide_rast[])
    
    # 3) values as 0 (non infected) and 1 (infected) cell
    #I_lide_rast[] <- ifelse(I_lide_rast[] > 0, 1, 0) 
    #I_lide_rast[] <- ifelse(I_lide_rast[] > 0, 1, NA) 
    
    #PLOT: overlay current plot on background image
    
    #bks <- c(0, 0.25, 0.5, 0.75, 1)
    #my_palette <- colorRampPalette(c("springgreen", "yellow1", "orange", "red1"))(n = 4)
    #image(I_lide_rast, breaks=bks, col=addalpha(my_palette, 1), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
    #bks <- seq(0, mx, length = 10)
    #image(I_lide_rast, breaks=bks, col=rev(heat.colors(length(bks)-1, alpha=1)), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
    #boxed.labels(xpos, ypos, tt, bg="white", border=NA, font=2)
    
    #WRITE TO FILE:
    #writeRaster(I_lide_rast, filename=paste('./', fOutput, '/', opt$output, '_', sprintf(formatting_str, cnt), sep=''), format='HFA', datatype='FLT4S', overwrite=TRUE) # % infected as output
    #writeRaster(I_lide_rast, filename=paste('./', fOutput, '/', opt$output, '_', sprintf(formatting_str, cnt), sep=''), format='HFA', datatype='INT1U', overwrite=TRUE) # nbr. infected hosts as output
    #writeRaster(I_lide_rast, filename=paste('./', fOutput, '/', opt$output, '_', sprintf(formatting_str, cnt), sep=''), format='HFA', datatype='LOG1S', overwrite=TRUE)  # 0=non infected 1=infected output
    
    
    ##FOR INFECTED UMCA OUTPUTS!
    ##CALCULATE OUTPUT TO PLOT: 
    # 1) values as % infected
    #I_umca_rast[] <- ifelse(I_umca_rast[] == 0, NA, I_umca_rast[]/umca_rast[])
    
    # 2) values as number of infected per cell
    #I_umca_rast[] <- ifelse(I_umca_rast[] == 0, NA, I_umca_rast[])
    
    # 3) values as 0 (non infected) and 1 (infected) cell
    #I_umca_rast[] <- ifelse(I_umca_rast[] > 0, 1, 0) 
    #I_umca_rast[] <- ifelse(I_umca_rast[] > 0, 1, NA) 
    
    #PLOT: overlay current plot on background image
    
    #bks <- c(0, 0.25, 0.5, 0.75, 1)
    #my_palette <- colorRampPalette(c("springgreen", "yellow1", "orange", "red1"))(n = 4)
    #image(I_umca_rast, breaks=bks, col=addalpha(my_palette, 1), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
    #bks <- seq(0, mx, length = 10)
    #image(I_umca_rast, breaks=bks, col=rev(heat.colors(length(bks)-1, alpha=1)), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
    #boxed.labels(xpos, ypos, tt, bg="white", border=NA, font=2)
    ...
    #integer matrix
    set.seed(42)
    spores_mat <- SporeGenCpp_MH(I_umca, I_lide, wLst, W, rate = spore_rate) #rate: spores/week for each infected host (4.4 default)
    ...
    if (wind == 'YES') {
      ...
      out <- SporeDispCppWind_MH(spores_mat, S_UM=S_umca, S_LD=S_lide, S_OK=S_oaks, I_UM=I_umca, I_LD=I_lide, I_OK=I_oaks, N_LVE=N_live, 
                                 W, rs=res_win, rtype='Cauchy', scale1=20.57, wdir=pwdir, kappa=2)
    
    }else{
      out <- SporeDispCpp_MH(spores_mat, S_UM=S_umca, S_LD=S_lide, S_OK=S_oaks, I_UM=I_umca, I_LD=I_lide, I_OK=I_oaks, N_LVE=N_live,
    ...
    if (cnt %% nth_output == 0){
      ...
      #bks <- c(0, 0.25, 0.5, 0.75, 1)
      #my_palette <- colorRampPalette(c("springgreen", "yellow1", "orange", "red1"))(n = 4)
      #image(I_lide_rast, breaks=bks, col=addalpha(my_palette, .5), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
      bks <- seq(0, mx, length = 10)
      image(I_lide_rast, breaks=bks, col=rev(heat.colors(length(bks)-1, alpha=1)), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
      boxed.labels(xpos, ypos, tt, bg="white", border=NA, font=2)
    ...
    #I_umca_rast[] <- ifelse(I_umca_rast[] > 0, 1, NA) 
      ...
      #bks <- c(0, 0.25, 0.5, 0.75, 1)
      #my_palette <- colorRampPalette(c("springgreen", "yellow1", "orange", "red1"))(n = 4)
      #image(I_umca_rast, breaks=bks, col=addalpha(my_palette, .5), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
      bks <- seq(0, mx, length = 10)
      image(I_umca_rast, breaks=bks, col=rev(heat.colors(length(bks)-1, alpha=1)), add=T, axes=F, box=F, ann=F, legend=F, useRaster=T)
      boxed.labels(xpos, ypos, tt, bg="white", border=NA, font=2)
